...
if [ -d .git ]; then
  # Perbaiki permission .git SELURUHNYA bila rusak oleh operasi root sebelumnya
  # (gejala: "insufficient permission for adding an object to repository database").
  # Jalankan SEBELUM git fetch — bila permission rusak, git fetch akan gagal.
  if [ -w .git/objects ] && [ -w .git/refs ]; then :; else
    echo "    Memperbaiki permission .git (rusak oleh operasi root)..."
